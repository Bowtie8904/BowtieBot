 ...
 *
 */
public class GetYoutubeChannelFeedCommand extends Command
{
    private Bot bot;
    private Main main;
     ...
     * @param permission
     */
    public GetYoutubeChannelFeedCommand(String[] validExpressions, int permission, Bot bot, Main main)
    {
     ...
     * @param permission
     */
    public GetYoutubeChannelFeedCommand(List<String> validExpressions, int permission, Bot bot, Main main)
    {
        super(validExpressions, permission, true);
        this.bot = bot;
        this.main = main;
    ...
    public void execute(CommandEvent event)
    {
        ...
        link = FeedUtils.getYouTubeFeedLink(link);

        if (link == null)
        {
            this.bot.sendMessage("Make sure to only use the URL of the channel page.",
                    event.getMessage().getChannel(), Colors.RED);
            return;
        }
    ...
    public EmbedObject getHelp(GuildObject guild)
    {
        ...
        builder.withAuthorName("Subscribe to a YouTube RSS Feed Command");
        builder.withAuthorUrl("http://www.bowtiebots.xyz/_commands_.html#rss");
        builder.withAuthorIcon(Main.BOWTIE_BOT_ICON);

        builder.withDescription("|  This command will subscribe to the  RSS feed of the\n"
                + "|  given YouTube channel.\n"
                + "|  Future video links of that channel will be sent to the given channel.\n"
        ...
        builder.appendField(
                "|  Usage:",
                "|  " + guild.getPrefix() + "youtubefeed <channel url> #channel\n"
                        + "|  " + guild.getPrefix()
                        + "youtubefeed https://www.youtube.com/channel/UClcE-kVhqyiHCcjYwcpfj9w #rss\n"
                        + "|", false);
