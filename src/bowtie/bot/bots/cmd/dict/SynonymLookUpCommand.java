 ...
 *
 */
public class SynonymLookUpCommand extends Command
{
    private Bot bot;
    private Main main;
     ...
     * @param permission
     */
    public SynonymLookUpCommand(String[] validExpressions, int permission, Bot bot, Main main)
    {
    ...
    public SynonymLookUpCommand(List<String> validExpressions, int permission, Bot bot, Main main)
    {
        super(validExpressions, permission, true);
        this.bot = bot;
        this.main = main;
    ...
    public void execute(CommandEvent event)
    {
        ...
        if (word.isEmpty())
        {
            bot.sendMessage("You have to specify a word. Take a look at the command description by typing '"
                    + event.getGuildObject().getPrefix() + "help syn'.", event.getChannel(), Colors.RED);
            return;
        }
        ...
        try
        {
            col = Dictionary.get().findSynonyms(word);
        }
        catch (IOException e)
        {
    ...
    public EmbedObject getHelp(GuildObject guild)
    {
        ...
        builder.withAuthorName("Synonym Command");
        builder.withAuthorUrl("http://www.bowtiebots.xyz/_commands_.html#dict");
        builder.withAuthorIcon(Main.BOWTIE_BOT_ICON);

        builder.withDescription("|  This command will show up to 20 synonyms for\n"
                + "|  the given word.\n"
                + "|  \n"
        ...
        builder.appendField(
                "|  Usage:",
                "|  " + guild.getPrefix() + "syn cold\n"
                        + "|  " + guild.getPrefix() + "syn high\n"
                        + "|", false);
