...
public class CreateListCommand extends Command
{
     ...
     * @param permission
     */
    public CreateListCommand(String[] validExpressions, int permission, Bot bot, Main main)
    {
    ...
    public CreateListCommand(List<String> validExpressions, int permission, Bot bot, Main main)
    {
        super(validExpressions, permission, true);
        this.bot = bot;
        this.main = main;
    ...
    public void execute(CommandEvent event)
    {
        ...
        {
            this.bot.sendMessage("You have to add the name of the list. Example: '"
        ...
        ItemList list = new ItemList(name, manager, dateString);

        if (main.getDatabase().existingGroup(event.getGuildObject().getStringID(), name))
        {
            this.bot.sendMessage(
                    "There is already a group with that name. A list can not have the same name as a group.",
                    event.getMessage().getChannel(), Colors.RED);
            return;
        }
        ...
        if (manager.add(list))
        {
            if (destroyTime != -1)
            {
                list.destroyAt(System.currentTimeMillis() + destroyTime);
                main.getDatabase().addListTimer(event.getGuildObject().getStringID(), name,
        ...
        {
            bot.sendMessage("That list does already exist.", event.getChannel(), Colors.RED);
        }
    }
    ...
    public EmbedObject getHelp(GuildObject guild)
    {
        ...
        builder.withAuthorName("Create List Command");
        builder.withAuthorUrl("http://www.bowtiebots.xyz/_commands_.html#collections");
        builder.withAuthorIcon(Main.BOWTIE_BOT_ICON);

        builder.withDescription("|  This command will create a new list with \n"
        ...
        builder.appendField(
                "|  Usage:",
                "|  " + guild.getPrefix() + "list listname\n"
                        + "|  " + guild.getPrefix() + "list animals\n"
                        + "|", false);
