 ...
 *
 */
public class VoiceChannelMoveHandler implements IListener<UserVoiceChannelMoveEvent>
{
    private Main main;
    private Pattern pat = Pattern.compile("(<[\\w|1-9\\s=]+>)");
    private SplittableRandom r;

    public VoiceChannelMoveHandler(Main main)
    {
        this.main = main;
    ...
    {
        if ((event.getNewChannel().isConnected() || event.getOldChannel().isConnected())
                && !event.getUser().isBot())
        {
            try
            {
            ...
            {
                text = main.getDatabase().getJoinText(event.getGuild().getStringID());
            }
            else if (event.getOldChannel().isConnected())
            {
                text = main.getDatabase().getLeaveText(event.getGuild().getStringID());
            }
