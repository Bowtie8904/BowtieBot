...
public class ReadyHandler extends BotReadyHandler
{
    ...
    public void prepare()
    {
        ...
        while (this.bot.getGuildObjects().size() != this.bot.getClient().getGuilds().size())
        {
            ...
            loadBannedUsers();
            loadCommandHandlers();
            main.setHomeGuild(bot.getGuildObjectByID(Properties.getValueOf("home")).getGuild());
            main.setTempChannel(bot.getClient().getChannelByID(
                    Long.parseLong(Properties.getValueOf("tempPremiumChannel"))));
            checkGroupTimers();
            checkListTimers();
        ...
        statusHandler.setDynamicUpdateHandler(() ->
        {
            ...
            playingTexts.add(new Presence(StatusType.ONLINE, ActivityType.PLAYING, this.bot.getGuildCount()
                    + " servers"));
